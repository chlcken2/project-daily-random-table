...
public class GeminiClient {
    ...
    private String buildNormalizationPrompt(String ingredientPhrase) {
        return "You are an ingredient normalization engine.\n"
                + "Task: Convert any ingredient phrase into a single standardized Japanese ingredient name.\n"
                + "Rules:\n"
                + "- Remove quantity numbers (1, 2, 3.5 etc.)\n"
                + "- Remove Japanese unit words (個, 本, 枚, 杯, 大さじ, 小さじ, g, ml など)\n"
                + "- Remove size expressions (大きい, 小さい, 大, 小 など)\n"
                + "- Remove cooking states (みじん切り, 千切り, 輪切り, 刻み, スライス など)\n"
                + "- Remove adjectives (新鮮な, 有機, etc.)\n"
                + "- Return only the core ingredient name in Japanese.\n"
