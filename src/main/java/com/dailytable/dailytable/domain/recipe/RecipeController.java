...
public class RecipeController {
     ...
     * RecipeDetailDto: 재료, 조리법, 영양정보 포함
     */
    @GetMapping("/{id}")
    ...
    public ResponseEntity<ApiResponse<Void>> toggleLike(
            @PathVariable Long id,
            @AuthenticationPrincipal Long userId) {
        log.info("POST /api/recipes/{}/like, userId: {}", id, userId);
        recipeService.toggleLike(id, userId);
        return ResponseEntity.ok(ApiResponse.success("「いいね」しました！", null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> addComment(
            ...
            @AuthenticationPrincipal Long userId) {
        log.info("POST /api/recipes/{}/comments, userId: {}", id, userId);
        String content = request.getContent();
        recipeService.addComment(id, userId, content);
        return ResponseEntity.ok(ApiResponse.success("コメントを投稿しました！", null));
    }
    ...
    public ResponseEntity<ApiResponse<List<CommentDto>>> getComments(
            @PathVariable Long id) {
        log.info("GET /api/recipes/{}/comments", id);
        List<CommentDto> comments = recipeService.getComments(id);
    ...
    public ResponseEntity<ApiResponse<Void>> cancelLike(
            @PathVariable Long id,
            @AuthenticationPrincipal Long userId) {
        log.info("DELETE /api/recipes/{}/like, userId: {}", id, userId);
        recipeService.cancelLike(id, userId);
        return ResponseEntity.ok(ApiResponse.success("「いいね」を取り消しました。", null));
    }
}
